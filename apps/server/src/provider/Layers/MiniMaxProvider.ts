...
> {
  return Effect.gen(function* () {
    ...
    // This endpoint reads a Token Plan Key only when the administrator supplied
    // one as a sensitive provider environment variable. It never starts an ACP
    // session or sends a prompt.
    const usageLimits = yield* readMiniMaxTokenPlanUsageLimits({
      enabled: settings.enabled,
      environment,
    });
