/**
 * MiniMaxAdapterLive — AWS MiniMax CLI (`mcode acp`) over ACP.
 *
...
import {
  currentMiniMaxModelIdFromSessionSetup,
...
export interface MiniMaxAdapterLiveOptions {
   ...
   */
  readonly resolveSettings?: Effect.Effect<MiniMaxSettings>;
}
...
interface MiniMaxSessionContext {
...
export function makeMiniMaxAdapter(
  minimaxSettings: MiniMaxSettings,
  options?: MiniMaxAdapterLiveOptions,
) {
  return Effect.gen(function* () {
    const boundInstanceId = options?.instanceId ?? ProviderInstanceId.make("minimax");
    const path = yield* Path.Path;
    const childProcessSpawner = yield* ChildProcessSpawner.ChildProcessSpawner;
    const crypto = yield* Crypto.Crypto;
    const makeAcpNativeLoggers = yield* makeAcpNativeLoggerFactory();

    const sessions = new Map<ThreadId, MiniMaxSessionContext>();
     ...
     */
    const stopSessionInternal = (ctx: MiniMaxSessionContext): Effect.Effect<void> =>
      Effect.gen(function* () {
    ...
    /**
     * Start a MiniMax session for a thread.
     *
    ...
    > =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const model = resolveMiniMaxAcpModelId(modelSelection?.model);
          ...
          const acp = yield* makeMiniMaxAcpRuntime({
            minimaxSettings: effectiveSettings,
              ...
              Effect.gen(function* () {
                ...
                yield* offerRuntimeEvent(
                  makeAcpRequestOpenedEvent({
                    ...
                    permissionRequest,
                    detail: permissionRequest.detail ?? "MiniMax requested permission.",
                    args: params,
                    source: "acp.jsonrpc",
          ...
          yield* acp
            .setConfigOption("permissionMode", miniMaxPermissionMode(input.runtimeMode))
            .pipe(
              Effect.mapError(
                (cause) =>
                  new ProviderAdapterProcessError({
          ...
          const reportedModel = currentMiniMaxModelIdFromSessionSetup(started.sessionSetupResult);
          ...
          const activeModel = model || reportedModel;
    ...
    > =>
      Effect.gen(function* () {
        ...
        if (!text) {
          // mcode has no promptless continuation, which is why
          ...
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "sendTurn",
            issue: "MiniMax requires prompt text; it does not support promptless continuation.",
          });
        }
        ...
        const requestedModel =
          input.modelSelection?.instanceId === boundInstanceId
            ? resolveMiniMaxAcpModelId(input.modelSelection.model)
            : undefined;
        if (requestedModel && requestedModel !== ctx.session.model) {
          yield* ctx.acp.setModel(requestedModel).pipe(
            Effect.mapError(
              (cause) =>
                new ProviderAdapterProcessError({
                  provider: PROVIDER,
    ...
    const requireSession = (
      threadId: ThreadId,
    ): Effect.Effect<MiniMaxSessionContext, ProviderAdapterSessionNotFoundError> =>
      Effect.suspend(() => {
        const ctx = sessions.get(threadId);
        if (!ctx || ctx.stopped) {
          return Effect.fail(
     ...
     *
     * This is the adapter's own record, not a query to mcode: ACP has no
     * "read history" call, and mcode's session store is its own business.
     ...
     * Rejected, always.
     *
     * mcode's ACP session cannot rewind its conversation, and
    ...
    const rollbackThread = (
      ...
      Effect.fail(
        new ProviderAdapterValidationError({
          ...
          issue: `MiniMax cannot roll back its conversation, so refusing to rewind ${numTurns} turn(s) on thread ${threadId}. Start a new thread instead.`,
        }),
      );
    ...
    return {
      provider: PROVIDER,
      capabilities: {
        // MiniMax exposes the model as a standard ACP session config option.
